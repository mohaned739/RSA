...
namespace RSA_Project
{
    public class Big_int
    {
        ...
        //T(N)=T(N/2) + N ^ 1.5
        //a=1 b=2
        //compare N^Log a base b with fn
        // N^0 < N^1.5
        // Master case 3
        //N^1.5=upper (n^(0+ e)) such that e is epsilon
        //First condition :  0<e<=1.58
        //second condition : (N^1.58) /2 <= c (N^1.58)
        //0.5 < c <= 1
        //complexity : O(N^1.58)


        public static Big_int encr(Big_int n, Big_int e, Big_int m)
        {
            Big_int res = new Big_int(n.arr.Length);//O(1)
            Big_int q = new Big_int(n.arr.Length);//O(1)
            Big_int r = new Big_int(n.arr.Length);//O(1)
            Big_int.div(m, n, ref q, ref r);//O(N)
            Big_int.pow_mod(r, e, n, ref res);//O(N^1.58)
